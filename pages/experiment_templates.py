...
def experiment_templates_page():
    ...
    selected_category = st.selectbox(
        "Choose a category:", list(templates.keys()))
    selected_template = st.selectbox(
        "Choose a template:", list(templates[selected_category].keys()))
    ...
    if selected_template not in st.session_state['template_versions']:
        st.session_state['template_versions'][selected_template] = [
            templates[selected_category][selected_template]]

    if selected_template not in st.session_state['template_comments']:
        st.session_state['template_comments'][selected_template] = []

    new_template_version = st.text_area(
        "Modify Template:", templates[selected_category][selected_template])
    st.write("### Template Preview")
    st.markdown(new_template_version)

    if st.button("Save Version"):
        st.session_state['template_versions'][selected_template].append(
    ...
    if st.button("Add Comment"):
        st.session_state['template_comments'][selected_template].append(
            version_comments)
    ...
    if st.button("Export Template"):
        template_str = '\n'.join(
            st.session_state['template_versions'][selected_template])
        st.download_button("Download Template", template_str,
                           f"{selected_template}.txt")
